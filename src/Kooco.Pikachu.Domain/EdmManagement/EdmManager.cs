...
{
    public async Task<Edm> CreateAsync(EdmTemplateType templateType, Guid? campaignId, EdmMemberType memberType,
    ...
    {
        var edm = new Edm(GuidGenerator.Create(), templateType, campaignId, memberType, memberTags,
    ...
    public async Task<Edm> UpdateAsync(Edm edm, EdmTemplateType templateType, Guid? campaignId, EdmMemberType memberType,
        IEnumerable<string> memberTags, bool applyToAllGroupBuys, IEnumerable<Guid> groupBuyIds, DateTime startDate,
        DateTime? endDate, DateTime sendTime, EdmSendFrequency? sendFrequency, string subject, string message)
    {
        ...
        edm.SetMemberType(memberType, memberTags);
